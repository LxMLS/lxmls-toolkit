...
class GaussianNaiveBayes(lc.LinearClassifier):
    ...
    def __init__(self):
        # Note: We get all methods (to test, evaluate) from the standard linear classifier. The only thing we need to design a train method for the Naive Bayes classifier.
    ...
    def train(self,x,y):
        ...
        # Estimation of classifier parameters (see quation 1.8) 
        ...
        for i in xrange(nr_c):
            ...
            # Estimate i^th mean from the sample mean. But only of the elements of the input set belonging to the i^th class!.
            # Note: np.mean is vectorized i.e. computes the nr_x means simultaneously
