...
class NaiveBayes(lc.LinearClassifier):
    ...
    def train(self, x, y):
        ...
        if self.xtype == "gaussian":
            ...
            raise ValueError(
                "Naive Bayes not implemented for this type of model of P(X|Y)"
            )

        self.trained = True
        return params

    # Estimate mean and variance of Gaussian Distributions
    # Note that the variance is shared for all classes
    def train_gaussian(self, x, y, nr_x, nr_f, nr_c):
        ...
        for i in range(nr_c):
            params[0, i] = -1 / (2 * variances[i]) * np.dot(
                means[i, :], means[i, :]
            ) + np.log(prior[i])
    ...
    # Train a gaussian distribution has one multinomial for each feature
