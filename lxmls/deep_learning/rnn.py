...
class RNN():
    ...
    def __init__(self, n_words, n_emb, n_hidd, n_tags):

        # MODEL PARAMETERS
        W_e = np.random.uniform(size=(n_emb, n_words))  # Input layer 
...
class LSTM():
    ...
    def __init__(self, n_words, n_emb, n_hidd, n_tags):
        ...
        # MODEL PARAMETERS
        W_e = np.random.uniform(size=(n_emb, n_words))  # Word embeeding layer 
        W_x = np.random.uniform(size=(n_hidd, n_emb))   # RNN Input layer
        W_h = np.random.uniform(size=(n_hidd, n_hidd))  # RNN Recurrent layer 
        W_c = np.random.uniform(size=(n_hidd, n_hidd))  # Second recurrent var 
        W_y = np.random.uniform(size=(n_tags, n_hidd))  # Output layer
        # Cast to theano GPU-compatible type
        W_e = W_e.astype(theano.config.floatX)
        W_x = W_x.astype(theano.config.floatX)
        ...
        # Class variables
        self.n_hidd = n_hidd
        self.param  = [_W_x, _W_h, _W_c, _W_y]
    ...
    def _forward(self, _x, _h0=None, _c0=None):
        ...
        # COMPUTATION GRAPH
        _W_x, _W_h, _W_c, _W_y = self.param
        H                      = self.n_hidd
        ...
        def _step(_x_tm1, _h_tm1, _c_tm1, _W_x, _W_h, _W_c):
            ...
            # GATES
            # Note the subtlety: _x_tm1 and hence _z_x are flat and have size (I,),
