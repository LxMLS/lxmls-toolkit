...
from lxmls.deep_learning.rnn import get_rnn_loss_range, get_rnn_parameter_handlers
...
def test_numpy_rnn(data):
    model = NumpyRNN(
        input_size=data.input_size, embedding_size=50, hidden_size=20, output_size=data.output_size, learning_rate=0.1
    )
    shape = [x.shape for x in model.parameters]
    assert np.allclose(shape, [(4786, 50), (20, 50), (20, 20), (12, 20)], tolerance)

    # Get functions to get and set values of a particular weight of the model
    get_parameter, set_parameter = get_rnn_parameter_handlers(layer_index=-1, row=0, column=0)
