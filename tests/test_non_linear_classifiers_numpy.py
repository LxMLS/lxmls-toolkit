...
def test_numpy_log_linear(corpus, data):
    class NumpyLogLinear(Model):
        ...
        def predict(self, *args, **kwargs):
            ...
            input = (
                kwargs.get("input") if "input" in kwargs else args[0] if args else None
            )
    ...
    model = NumpyLogLinear(
        input_size=corpus.nr_features, num_classes=2, learning_rate=learning_rate
    )
...
def test_backpropagation_numpy(corpus, data):
    ...
    get_parameter, set_parameter = get_mlp_parameter_handlers(
        layer_index=1, is_bias=False, row=0, column=0
    )
    ...
    weight_range, loss_range = get_mlp_loss_range(
        model, get_parameter, set_parameter, batch
    )
